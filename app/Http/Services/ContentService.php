...
class ContentService
{
    ...
    {
        $query = Review::select('tb_reviews.*')
            ->selectRaw('tb_cms_posts.title')
            ->leftJoin('tb_cms_posts', 'tb_cms_posts.id', '=', 'tb_reviews.id_product')

            ->when(!empty($params['keyword']), function ($query) use ($params) {
                $keyword = $params['keyword'];
                return $query->where(function ($where) use ($keyword) {
                    return $where->where('tb_reviews.name', 'like', '%' . $keyword . '%')
                        ->orWhere('tb_reviews.email', 'like', '%' . $keyword . '%');
                });
            })

            ->when(!empty($params['status']), function ($query) use ($params) {
                return $query->where('tb_reviews.status', $params['status']);
            })
            ->when(!empty($params['created_at_from']), function ($query) use ($params) {
                $query->where('tb_reviews.created_at', '>=', $params['created_at_from']);
            })
            ->when(!empty($params['created_at_to']), function ($query) use ($params) {
                $query->where('tb_reviews.created_at', '<=', $params['created_at_to']);
            });
        return $query;
    }
    ...
    public static function getOrderDetail($params)
    {
        $query = OrderDetail::select('tb_order_details.*')
            ...
            ->selectRaw('tb_cms_taxonomys.title AS taxonomy_title')
            ->join('tb_cms_posts', 'tb_cms_posts.id', '=', 'tb_order_details.item_id')
