...
@push('style')
    <link rel='stylesheet' href="{{ asset('themes/frontend/divine/css/bootstrap.min.css') }}" media='all' />
    <link rel="stylesheet" href="{{ asset('themes/frontend/divine/css/slick.css') }}" type="text/css" />
    <link rel="stylesheet" href="{{ asset('themes/frontend/divine/css/slick-theme.css') }}" type="text/css" />
    <style>
...
@section('content')
            ...
            <div class="row gx-5">
                <aside class="col-lg-6 box_slide_image mb-3 mb-lg-0">
                    <div class="border rounded-4  d-flex justify-content-center align-items-center slider slider-for">
                        <a data-fslightbox="mygalley" class="rounded-4"data-type="image" href="{{ $image }}">
                            <img style="max-width: 100%; max-height: 100vh; margin: auto;" class="rounded-4 fit"
                                src="{{ $image }}" />
                        </a>
                        @if (isset($detail->json_params->gallery_image))
                            @foreach ($detail->json_params->gallery_image as $img)
                    ...
                    <div class="d-flex justify-content-center  slider slider-nav mt-25">
                        <div data-fslightbox="mygalley" class="mx-1 rounded-2" data-type="image" class="item-thumb">
                            <img width="150" height="150" class="rounded-2" src="{{ $image }}" />
                        </div>
                        @if (isset($detail->json_params->gallery_image))
                            @foreach ($detail->json_params->gallery_image as $img)
                                <div data-fslightbox="mygalley" class="mx-1 rounded-2" data-type="image" class="item-thumb">
                                    <img width="150" height="150" class="rounded-2" src="{{ $img }}" />
                                </div>
                            @endforeach
                        @endif
                    </div>
                ...
                <main class="col-lg-6">
                    <div class="ps-lg-3">
                        ...
                        <div class="mt-25">
                            <span class="salary">{{ number_format($price, '0', '', '.') }}
                                {{ App\Consts::CURRENCY_UNIT['vi'] }}</span>
                        </div>
                        @if ($image_thumb != null)
                        ...
                        <p class="box-type">
                            @lang('Trademark: ') <a href="{{ $alias_category }}">{{ $taxonomy_title }}</a>
                        </p>
                            ...
                            @if ($description != null)
                                <div class="nav-item" role="presentation">
                                    <a class="nav-link  active" id="tab-1" data-mdb-toggle="pill" href="#pills-1"
                                        role="tab" aria-controls="pills-1" aria-selected="true">@lang('Description')</a>
                                </div>
                            @endif
                            @if ($content != null)
                                <div class="nav-item" role="presentation">
                                    <a class="nav-link" id="tab-2" data-mdb-toggle="pill" href="#pills-2"
                                        ...
                                        aria-selected="false">@lang('Product Details')</a>
                                </div>
                            @endif
                            @if ($information != null)
                                <div class="nav-item" role="presentation">
                                    <a class="nav-link" id="tab-3" data-mdb-toggle="pill" href="#pills-3"
                                        ...
                                        aria-selected="false">@lang('Information')</a>
                                </div>
                            @endif
                            @if ($using != null)
                                <div class="nav-item" role="presentation">
                                    <a class="nav-link" id="tab-4" data-mdb-toggle="pill" href="#pills-4"
                                        ...
                                        aria-selected="false">@lang('How to you')</a>
                                </div>
                            @endif
                        </div>
                        <!-- Pills navs -->
    ...
    @if (isset($relatedPosts) && count($relatedPosts) > 0)
        <div class="container elementor-kit-12">
            <h2 class="title-widget text-capitalize mt-5">@lang('products commonly used together')</h2>
                    ...
                    <div class="col-12 col-md-6 col-lg-3">
                        <div class="product">
                            <div class="product-image">
                                <a href="{{ $alias }}">
                                    <img src="{{ $image }}" alt="{{ $title_item }}">
                                </a>
                            </div>
                            <div class="product-desc center">
                                <div class="product-title page-content">
                                    <h3><a class="link_title" href="{{ $alias }}">{{ $title_item }}</a></h3>
                                </div>
                            </div>
                        </div>
                    </div>
...
@endsection
@push('script')
    <script src="{{ url('themes/frontend/divine/js/bootstrap.min.js') }}"></script>
    <script src="{{ asset('themes/frontend/divine/js/slick.min.js') }}"></script>
    <script src="{{ asset('themes/frontend/divine/js/script.min.js') }}"></script>
    <script>
